  ...
  proxyTool(
    ...
    "Create or update an autonomy task. Omit id to create; include id to update.",
    {
      ...
      properties: {
        ...
        pipeline_mode: { type: "boolean", description: "Enable pipeline execution mode (plan→implement→review)" },
        auto_advance: { type: "boolean", description: "Auto-advance recurring tasks back to up_next on completion" },
        runs_per_day: { type: "number", description: "Target executions per day (e.g., 288 = every 5min)" },
        depends_on: { type: "integer", description: "Task ID this task depends on" },
        notify_on_complete: { type: "boolean", description: "Send toast notification on completion (default true)" },
        max_retries: { type: "integer", description: "Max retry attempts before moving to inbox (default 3)" },
        preferred_hours: { type: "string", description: "JSON array of preferred UTC hours for execution" },
        preemptible: { type: "boolean", description: "Whether task can be preempted by higher priority work" },
        last_run: { type: "string", description: "ISO-8601 timestamp of last run" },
        cron_id: { type: "string", description: "Associated cron job ID" },
        activity_note: { type: "string", description: "Activity note to add to task history" },
        pipeline: { type: "string", description: "JSON array of pipeline stages (e.g. [\"plan\",\"implement\",\"review\"])" },
      },
    },
    5_000,
  );
