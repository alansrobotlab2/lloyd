...
- `http_search` — search the web via DuckDuckGo
- `http_fetch` — fetch and extract content from URLs
- `http_request` — make HTTP requests to APIs
- `browser` — interactive web browsing
- `qmd_search`, `qmd_get` — read-only vault access for existing knowledge context
- `read` — read local files and documents

## Workflow

1. Check the vault first with `qmd_search` to see if relevant notes already exist
