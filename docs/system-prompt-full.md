# Complete System Prompt — Lloyd (main agent)

> Reconstructed from `buildAgentSystemPrompt()` in
> `reply-Duq0R59W.js` (OpenClaw v2026.2.26) + workspace files.
> This is the full text sent as the system message to the LLM.

---

## 1. Framework Boilerplate (11,172 chars, ~2,793 tokens)
...
[Skills listing injected from workspace — see Section 3 below]
...
[Dynamic — depends on config. Typically lists aliases like 'opus', 'sonnet', etc.]

If you need the current date, time, or day of week, run session_status (📊 session_status).
## Workspace
Your working directory is: /home/alansrobotlab/obsidian/agents/lloyd
...
## Authorized Senders
Authorized senders: [owner phone/ID]. These senders are allowlisted; do not assume they are the owner.
...
Keep spoken text ≤500 chars to avoid auto-summary (summary on).
...
## Runtime
Runtime: agent=main | host=openclaw | os=Linux 6.18.x-arch1-2 (x64) | node=v22.x | model=local-llm/Qwen3.5-35B-A3B | default_model=anthropic/claude-sonnet-4-6 | shell=zsh | channel=main | capabilities=none | thinking=off
Reasoning: off (hidden unless on/stream). Toggle /reasoning; /status shows Reasoning when enabled.

```

## 2. Project Context — Workspace Files

These are injected verbatim as `# Project Context` → `## <filename>` sections.
...
Injected as `<available_skills>` XML within the Skills section.
Each skill has name, description, location (path to SKILL.md).
...
## 4. Tool Descriptions (JSON Schemas)

These are sent in the `tools` array of the chat completion request,
alongside the system message. Each tool has a name, description, and
...
```
  tag_search:
    ...
    params:
      ...
      type: hub|notes|project-notes|work-notes|talk|any
...
```
  vault_overview:
    ...
    params:
      detail: 'summary'|'tags'|'hubs'|'types' (default: 'summary')
```
...
```
  http_fetch:
    ...
    params:
      ...
      maxChars: integer (default: 50000)
```
...
```
  bg_process:
    ...
    params:
      action: 'list'|'poll'|'log'|'write'|'kill' (required)
      session_id: string
      timeout: integer
      text: string
      offset: integer
...
**Note:** Core built-in tools (read, write, edit, exec, process, etc.) have their
own JSON schemas defined in the framework. They add ~3-5k chars of schema definitions
to the `tools` array in the API request. These are not part of the system message text
but are counted as input tokens by the LLM tokenizer.
...
|-----------|-------|-------------|
| Framework boilerplate | 11,172 | ~2,793 |
| Workspace bootstrap files | 26,931 | ~6,733 |
| Workspace skills | 32,784 | ~8,196 |
| Tool descriptions (external) | 4,058 | ~1,015 |
| Core tool schemas (est.) | ~4,000 | ~1,000 |
| **Total** | **~78,945** | **~19,737** |

Actual measured input tokens from llama-server logs: **~21,200-23,100 tokens**

Token estimate discrepancy: char/4 underestimates because the LLM tokenizer
(Qwen 3.5) handles markdown, JSON schemas, and code differently than plain text.
The actual ratio is closer to ~3.2 chars/token for this mixed content.
