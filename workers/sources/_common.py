...
All these sources follow the same pattern:
  ...
  3. response lands under ~/lloyd-data/_pipeline/vault-derived/pending-research/{source}/{yyyy-mm-dd}/
...
# The self-modification loop and grant minting are not a worker's to touch.
# The lists live in `app/tool_bans.py` (review 2026-09-24, X4) so the
...
async def run_prompt_in_session(prompt: str, *, title: str, source: str,
    ...
    **The budget is wall-clock, and it has to be smaller than the pool's.**
    ...
    out: dict = {"text": "", "session_id": session_id, "stop_reason": None,
                 ...
                 "finalizer_reasoning_tokens": None}
            ...
            async with client.stream("POST", f"{backend}/api/message/stream",
                ...
                async for event, data in _aiter_sse(resp):
                    if event == "error":
                        ...
                        out["finalizer_reasoning_tokens"] = data.get(
                            "finalizer_reasoning_tokens")
                        break
