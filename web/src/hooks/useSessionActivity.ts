...
export function useSessionActivity(
  ...
  // Poll every 2s to re-evaluate activity
  useEffect(() => {
    if (!enabled) {
  ...
  // Initialize lastSeen for new sessions so they don't immediately show a dot
  useEffect(() => {
    const now = Date.now()
  ...
  const hasActivity = useCallback(
    (sessionKey: string): boolean => {
      void tick // reference tick so the callback refreshes on each poll
    ...
    [sessions, lastSeen, currentSessionKey, tick],
  )
