...
~8,700 tests under `tests/`, run by every automod round's `tests` rung
...
**Live-data** tests read what is *on the machine* — `sessions/`, `_pipeline/`,
`eval/baselines/`, `_pipeline/vault-derived/kg.sqlite`. They exist for a real
...
`test_promotion_fp_rate.py` is the shape to copy — 22 synthetic tests covering
the derivation, and the live cross-checks kept separate and clearly marked.
...
- **No** — the check is pinned to a *frozen window*: the FP-rate note's window
  ending `R_20260908_181458`, the thirteen `nightly-2026090*.json` of 09-04..09-17,
