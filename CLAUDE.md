...
- `memory_search` — QMD semantic/hybrid vector search across `~/obsidian/**/*.md`
- `memory_get` — QMD file reader by relative path (with optional line range)
...
All return the same relative paths. `tag_search` results are retrievable via `memory_get`.
