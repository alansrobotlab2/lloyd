...
**Reading (coordinator, 2026-09-25): keep  at 0.72 / 0.52; no config change.**
The runner's strict rule returns *inconclusive* because the  shape
"fails at the base length". That failure is not rot: 10 of the 12 misses in
the whole run are / cells graded , where the
...
one thing a rot curve would not be. Every  and  cell is
1.0 up to 240k in both shapes (one multi3 miss at 240k), and  holds
≥ 0.93 everywhere. Nothing here says Qwen3.8 degrades before the window
ends, so the plan's  branch is the honest reading.
...
Follow-up if the distractor condition should gate a future run: raise its
 or ask for the port alone, so hedging cannot read as a miss.
Raw rows: .
