...
# kernel path: /home/alansrobotlab/.cache/vllm/torch_compile_cache/torch_aot_compile/5b9a108a067e3067201b5ef792914c1bf0174396f4045714a126bf46a3b39ff8/inductor_cache/va/cvas3nkxmvnkixb5akff7fijdcd657gtbfxuqbbtwdp4acjy7lln.py
...
#   %arg2_1 : Tensor "i32[1][1]cuda:0" = PlaceHolder[target=arg2_1]
#   %arg6_1 : Tensor "i32[s34][1]cuda:0" = PlaceHolder[target=arg6_1]
...
#   %index_2 : Tensor "i32[s34][1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.index.Tensor](args = (%arg5_1, [%clamp_min]), kwargs = {})
...
#   %index_1 : Tensor "i32[s34][1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.index.Tensor](args = (%arg4_1, [%clamp_min]), kwargs = {})
#   %index : Tensor "i32[s34][1]cuda:0"[num_users=2] = call_function[target=torch.ops.aten.index.Tensor](args = (%arg2_1, [%clamp_min]), kwargs = {})
#   %add_14 : Tensor "i32[s34][1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%index_1, %index), kwargs = {})
#   %where : Tensor "i32[s34][1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.where.self](args = (%bitwise_and, %add_14, %arg6_1), kwargs = {})
#   %where_1 : Tensor "i32[s34][1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.where.self](args = (%bitwise_and, %index, %arg8_1), kwargs = {})
#   %copy__1 : Tensor "i32[s34][1]cuda:0"[num_users=0] = call_function[target=torch.ops.aten.copy_.default](args = (%arg8_1, %where_1), kwargs = {})
...
def triton_poi_fused_add_bitwise_and_clamp_copy__ge_gt_index_where_0(in_ptr0, in_ptr1, in_ptr2, in_ptr3, in_ptr4, in_ptr5, out_ptr0, out_ptr2, ks0, xnumel, XBLOCK : tl.constexpr):
    ...
    tmp23 = tl.load(in_ptr5 + (x0), xmask)
    ...
    tmp14 = tl.full([XBLOCK], 1, tl.int32)
    tmp15 = tmp3 + tmp14
    tmp16 = tl.where(tmp6, tmp15, tmp3)
    tl.device_assert(((0 <= tmp16) & (tmp16 < 1)) | ~(xmask), "index out of bounds: 0 <= tmp16 < 1")
    tmp20 = tmp13 + tmp19
    tmp22 = tl.where(tmp12, tmp20, tmp21)
    tmp24 = tl.where(tmp12, tmp19, tmp23)
    tl.store(out_ptr0 + (x0), tmp22, xmask)
    tl.store(out_ptr2 + (x0), tmp24, xmask)
''', device_str='cuda')


# kernel path: /home/alansrobotlab/.cache/vllm/torch_compile_cache/torch_aot_compile/5b9a108a067e3067201b5ef792914c1bf0174396f4045714a126bf46a3b39ff8/inductor_cache/qd/cqdljunmdeddev4fo7fopzaiewtcc4ujhpmg34brdd4vpj4i227v.py
...
#   %add_14 : Tensor "i32[s34][1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%index_1, %index), kwargs = {})
#   %where : Tensor "i32[s34][1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.where.self](args = (%bitwise_and, %add_14, %arg6_1), kwargs = {})
#   %slice_tensor : Tensor "i32[s34][1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.slice.Tensor](args = (%arg4_1, 0, 0, %arg7_1), kwargs = {})
...
class Runner:
    ...
    def call(self, args):
        ...
        assert_size_stride(arg6_1, (s34, ), (1, ))
        assert_size_stride(arg8_1, (s34, ), (1, ))
        with torch.cuda._DeviceGuard(0):
            torch.cuda.set_device(0)
            buf2 = empty_strided_cuda((s34, ), (1, ), torch.int32)
            # Topologically Sorted Source Nodes: [ge, gather_indices, prev_drafts, gt, participating, prev_computed, valid_counts, corrected, where, where_1], Original ATen: [aten.ge, aten.clamp, aten.index, aten.gt, aten.bitwise_and, aten.add, aten.where, aten.copy_]
            stream0 = get_raw_stream(0)
            triton_poi_fused_add_bitwise_and_clamp_copy__ge_gt_index_where_0.run(arg1_1, arg5_1, arg4_1, arg2_1, arg6_1, arg8_1, buf2, arg8_1, s13, s34, stream=stream0)
            del arg1_1
            del arg2_1
            del arg5_1
            del arg6_1
            ...
            # Topologically Sorted Source Nodes: [ge, gather_indices, prev_drafts, gt, participating, prev_computed, valid_counts, corrected, where], Original ATen: [aten.ge, aten.clamp, aten.index, aten.gt, aten.bitwise_and, aten.add, aten.where]
            stream0 = get_raw_stream(0)
            triton_poi_fused_add_bitwise_and_clamp_ge_gt_index_where_1.run(buf2, arg4_1, s34, stream=stream0)
            del arg4_1
            del buf2
        return ()
...
def get_args():
    ...
    arg2_1 = rand_strided((1, ), (1, ), device='cuda:0', dtype=torch.int32)
    arg3_1 = 2
    ...
    arg7_1 = 2
    arg8_1 = rand_strided((2, ), (1, ), device='cuda:0', dtype=torch.int32)
