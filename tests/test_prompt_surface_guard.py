...
live `~/obsidian` vault. Those ran on the selfmod gate's hard `tests` rung, so
...
* `scripts/selfmod/vault_round.validate` — the loop's own vault route.
...
def test_gate_tests_rung_excludes_live_vault_assertions():
    ...
    src = (Path(__file__).resolve().parent.parent
           / "scripts" / "selfmod" / "gate.py").read_text(encoding="utf-8")
...
@pytest.mark.parametrize("path,fn", [
    ("scripts/selfmod/vault_round.py", "check_paths"),
