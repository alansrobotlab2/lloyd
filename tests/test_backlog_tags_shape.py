...
them said so:
  ...
  * ``scripts/automod/backlog.py`` ran ``[str(t) for t in fm["tags"]]``, which
...
from scripts.automod import backlog as SB
...
def test_normalize_tags_never_iterates_a_string_by_character():
    """The automod failure specifically: `[str(t) for t in "abc"]` is 3 tags."""
...
def test_automod_item_tags_are_whole_tags(boards):
...
def test_no_task_on_the_live_board_has_a_scalar_tags_field():
    ...
    automod gate (`-m "not live_vault"`) does not fail a round on a vault the
    round never touched."""
