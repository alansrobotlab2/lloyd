    ...
    return [
        ...
        Tool(
            ...
            description=(
                ...
                "\"Pull up X in the memory tab\", \"open task 42\", \"let's look "
                "at autonomy\" ask you to move the user's view, not to fetch the "
                "data into chat. Switches their Mission Control tab, optionally "
                "focuses one item in it, and returns what is now visible.\n\n"
                "focus_id by tab: inner_voice/chat a session id; backlog a task "
                "id (opens it) or board id; autonomy a task id; workers a source "
                "name; memory an entity name or a vault path such as "
                "\"lloyd/SOUL.md\"; tools an MCP server id; skills a skill name; "
                "services a unit name; ide an absolute file path (ide_open_file "
                "gives richer feedback); browser none (drive it with "
                "browser_navigate); desktop none (it mirrors desktop_capture and "
                "holds the lease Alan grants for desktop_act); architecture, "
                "settings, graph, dashboard none.\n\n"
                "An invalid focus_id still switches the tab and returns "
                "`focus_error`; find the right id (e.g. with vault_search) and "
                "retry the focus."
            ),
            inputSchema={
                "type": "object",
