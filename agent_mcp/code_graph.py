...
- `tests/test_mcp_layer.py` requires every configured server to be
  discoverable at test time — including inside a selfmod worktree, where a
  second daemon is not running.
- `agent-services/supervisor/**` is a protected selfmod path, so Lloyd
...
# A round id, as `scripts/selfmod/round.py` mints them.
...
def resolve_root(root: str | None) -> Path:
    ...
    links a chat session to an open selfmod round — `round_start` ledger
    ...
    if ROUND_ID_RE.match(raw):
        from scripts.selfmod.worktree import worktree_path
        p = worktree_path(raw)
        if not p.is_dir():
...
async def _dirty_sources_newer_than_graph(root: Path) -> list[str]:
    ...
    Mandatory, not a nicety: inside a selfmod round HEAD does not move
...
async def _open_worktrees() -> list[str]:
    """Selfmod worktrees currently checked out, cached for 10 s."""
    now = time.time()
...
async def _header_lines(root: Path, entry: Entry | None, st: Staleness,
    ...
    if os.path.realpath(root) == os.path.realpath(LLOYD_HOME):
        for wt in await _open_worktrees():
            lines.append(f"note: open selfmod worktree at {wt}; "
...
_ROOT_DESC = ("Tree to answer about. Omit for the live checkout; pass an "
              "SM_… round id or an absolute path to ask about a selfmod "
